...
# 여행 상태(진행중/다녀옴) 판정과 저장 시각은 '한국 시간'을 기준으로 한다.
# 서버가 UTC(예: Render)로 돌 때 datetime.now()를 쓰면 한국 새벽~오전엔 날짜가
# '어제'로 계산돼, 오늘 시작하는 여행이 '계획중'에 머무는 문제가 생긴다.
# 한국은 서머타임이 없어 항상 UTC+9 고정이므로 고정 오프셋으로 안전하게 계산한다.
...
# MongoDB 연결
# 배포 환경(Render 등)에는 로컬 MongoDB가 없으므로, 연결 문자열을 환경변수로 주입한다.
#   Render 대시보드 → Environment → MONGO_URI = mongodb+srv://<user>:<pw>@cluster0.xxxx.mongodb.net/?retryWrites=true&w=majority
# 환경변수가 없으면(내 컴퓨터에서 개발할 때) localhost로 폴백한다.
# 타임아웃은 Atlas 최초 연결/콜드스타트를 감안해 넉넉히 잡는다(기존 2s → 8s).
...
def _name_relevant(place_title, query):
    """장소 '이름'이 사용자가 친 검색어와 관련 있는지 판단한다.
       Kakao Local은 이름뿐 아니라 '주소'에 검색어가 들어가도 결과를 주기 때문에
       (예: '설악' 검색 → 주소가 '설악로'인 무관한 가게들), 이름 기준으로 걸러
       '검색어가 이름에 들어간 곳'만 남긴다."""
    qk = _name_key(query)
    tk = _name_key(place_title)
    if not qk or not tk:
        return False
    if qk in tk or tk in qk:
        return True
    # 검색어를 띄어쓰기로 나눈 토큰 중 하나라도(2자 이상) 이름에 들어가면 관련으로 본다.
    for tok in re.split(r"\s+", str(query or "")):
        tkk = _name_key(tok)
        if len(tkk) >= 2 and tkk in tk:
            return True
    return False
...
def kakao_verify_candidates(name, prov_short=None, sigungu_name=None):
    ...
    # 이름이 검색어와 무관한(주소만 걸린) 결과는 제외한다.
    found = [p for p in found if _name_relevant(p.get("title"), base)]
    return found[:10], errors[:3]


def pet_search_in_region(name, prov_short=None, sigungu_name=None, limit=8):
    """'둘러보기'와 동일한 풀(load_all_pets)에서 장소명을 직접 검색한다.

    검증(/api/tour/verify)은 원래 Kakao Local에서 장소를 먼저 '발견'한 뒤
    반려동물 동반여행 API와 대조했다. 그래서 Kakao에 없거나, 이름·주소 표기가
    달라 대조/지역필터에 걸러지면, 정작 둘러보기 목록(=반려동물 동반 API 전량)에는
    있는 장소가 '검색하면 없음'으로 나오는 문제가 있었다.
    여기서는 둘러보기가 쓰는 바로 그 풀을 이름으로 직접 뒤져,
    Kakao가 못 찾은 장소도 검증 결과에 반드시 포함되도록 한다."""
    from difflib import SequenceMatcher
    q = _name_key(name)
    if not q:
        return []
    ...
    for it in load_all_pets():
        # 둘러보기와 '같은' 주소 기준 지역 필터 (pets_in_region과 동일 규칙)
        if (prov_short or sigungu_name) and not address_matches_region(
                it.get("addr1", ""), prov_short, sigungu_name):
            continue
        title = _name_key(it.get("title"))
        if not title:
            continue
        if q == title:
            s = 1.0
            ...
            s = SequenceMatcher(None, q, title).ratio()
            if s < 0.62:            # 이름이 사실상 다른 장소는 제외
                continue
        scored.append((s, it))
    scored.sort(key=lambda x: x[0], reverse=True)
    return [norm_place(it) for _s, it in scored[:limit]]


def _merge_pet_hits(results, pet_hits, dog):
    """둘러보기 풀에서 직접 찾은 장소를 검증 결과에 합친다.

    - 결과에 없던 장소면 반려견 기준으로 판정(_enrich_one)해 추가한다.
    - 같은 장소가 이미 '동반 불가/미등록'으로 잡혀 있으면(예: Kakao로는 찾았으나
      반려동물 API 대조에 실패한 경우), 근거 있는 이 결과로 '교체'해
      둘러보기와 판정을 일치시킨다.
    - 이미 반려동물 API로 판정된 동일 장소면 중복이므로 그대로 둔다."""
    def _same(a_id, a_name, r):
        rid = str(r.get("contentid") or "")
        if a_id and rid and a_id == rid:
            return True
        rn = _name_key(r.get("title"))
        if a_name and rn and (a_name == rn or
                              (len(a_name) >= 3 and (a_name in rn or rn in a_name))):
            return True
        return False

    for ph in pet_hits:
        cid = str(ph.get("contentid") or "")
        nk = _name_key(ph.get("title"))
        dup = next((r for r in results if _same(cid, nk, r)), None)
        if dup is not None and dup.get("pet_registered"):
            continue                      # 이미 근거 있는 동일 장소 → 그대로 둠
        enriched = dict(ph)
        enriched["pet_registered"] = True
        _enrich_one(enriched, dog)
        if dup is None:
            results.append(enriched)
        else:
            results[results.index(dup)] = enriched
    return results


def _sort_verify_results(results, name):
    """검증 결과 정렬: 이름이 정확히 일치 → 포함 → 그 외, 그다음 판정이 좋은 순."""
    target = _name_key(name)
    order = {"ok": 0, "check": 1, "conditional": 1, "no": 2}
    results.sort(key=lambda p: (
        0 if _name_key(p.get("title")) == target else
        1 if target and target in _name_key(p.get("title")) else 2,
        order.get(p.get("verdict"), 3),
    ))
    return results


# 캠핑장 이름에 흔히 붙는 '종류' 단어들. 서로 다른 장소를 같은 곳으로 오인하지
# 않도록, 같은 장소 판별 시 이 단어들은 무시한다(예: '글램핑'만 겹치는 건 동일 아님).
_PLACE_TYPE_STOP = {
    "글램핑", "글램핑장", "캠핑", "캠핑장", "오토캠핑", "오토캠핑장", "야영장",
    "자동차야영장", "카라반", "펜션", "호텔", "모텔", "리조트", "카페", "펫",
    "반려견", "반려동물", "파크", "관광농원", "농원", "가든", "마을", "휴양림",
}


def _place_titles_same(t1, t2):
    """두 장소명이 사실상 같은 곳을 가리키는지 느슨하게 판단한다.
       - 정규화 이름이 같거나 한쪽이 다른 쪽에 포함되면 동일.
       - 그 외에는 '종류 단어'를 뺀 고유 토큰(3자 이상)이 상대 이름에 들어 있으면 동일.
         (예: '서울 서초 글램핑 청계산장' ↔ '청계산장글램핑' → '청계산장'으로 동일 판정)"""
    k1, k2 = _name_key(t1), _name_key(t2)
    if not k1 or not k2:
        return False
    ...
    def spec_tokens(t):
        out = []
        for piece in re.split(r"\s+", str(t or "")):
            tk = _name_key(piece)
            if len(tk) >= 3 and tk not in _PLACE_TYPE_STOP:
                out.append(tk)
        return out

    for x in spec_tokens(t1):
        if x in k2:
            return True
    for x in spec_tokens(t2):
        if x in k1:
            return True
    return False


def camp_search_in_region(name, prov_short=None, sigungu=None, dog=None, limit=8):
    """'둘러보기 캠핑'이 쓰는 고캠핑 풀(load_all_camps)에서 캠핑장 이름을 직접 검색한다.

    반려동물 동반 API에는 없고 고캠핑에만 있는 캠핑장(예: 지자체 등록 글램핑장)은
    기존 검증(pet 풀 + Kakao)에서 '등록된 장소가 아니어서 동반 불가'로 잘못 나왔다.
    둘러보기 캠핑이 보는 바로 그 풀을 직접 뒤져, 둘러보기와 판정을 일치시킨다."""
    from difflib import SequenceMatcher
    q = _name_key(name)
    if not q:
        return []
    ...
    for it in load_all_camps():
        short, _code = map_doname(it.get("doNm"))
        if prov_short and short != prov_short:
            continue
        if want and not sig_match(it.get("sigunguNm"), want):
            continue
        title = _name_key(it.get("facltNm"))
        if not title:
            continue
        if q == title:
            s = 1.0
    ...
    out = []
    for _s, it in scored[:limit]:
        c = norm_camp(it, dog)
        v = c.get("verdict") or "no_info"
        c["reason"] = (
            "등록하신 반려견과 함께 갈 수 있는 캠핑장이에요." if v == "ok"
            else "이 캠핑장은 반려동물 동반이 불가합니다." if v == "no"
            else "일부 조건이 있는 캠핑장이에요. 방문 전 전화로 확인하는 걸 권장합니다."
            if v in ("check", "conditional")
            else "고캠핑 데이터에 동반 가능 여부가 기재돼 있지 않아 확인이 필요해요. "
                 "방문 전 전화 확인을 권장합니다.")
        c["overview"] = c.get("intro", "")
        if c.get("facility"):
            c["extra"] = f'<span class="k">부대시설</span> · {c["facility"]}'
        c["source"] = "gocamping"
        out.append(c)
    return out


def _merge_camp_hits(results, camp_hits):
    """고캠핑 풀에서 찾은 캠핑장을 결과에 합친다.
       (판정을 그대로 보존해야 하므로 _enrich_one을 태우지 않는다 —
        고캠핑 장소는 contentid가 없어 _enrich_one을 태우면 판정이 뭉개진다.)"""
    have = {_name_key(r.get("title")) for r in results if _name_key(r.get("title"))}
    for c in camp_hits:
        nk = _name_key(c.get("title"))
        if nk and any(nk == h or (len(nk) >= 3 and (nk in h or h in nk))
                      for h in have):
            continue
        if nk:
            have.add(nk)
...
def _suppress_dup_no_cards(results):
    """같은 장소가 '동반 가능/확인 필요'로도, (Kakao발) '동반 불가'로도 잡혔을 때
       모순되는 '동반 불가' 카드를 제거한다. → 둘러보기와의 판정 충돌 방지."""
    good = [r for r in results if r.get("verdict") in ("ok", "check", "conditional")]
    if not good:
        return results
    out = []
    for r in results:
        if r.get("verdict") == "no" and any(
                _place_titles_same(r.get("title"), g.get("title")) for g in good):
            continue
        out.append(r)
    return out
...
def tour_verify():
    ...
    # 둘러보기(반려동물 동반 API 전량)에서 장소명을 직접 검색한 결과.
    # Kakao 발견 여부와 무관하게, 둘러보기에 보이는 곳은 검증에서도 반드시 잡히도록 한다.
    pet_hits = pet_search_in_region(name, prov_short, sigungu_name, limit=8)
    # 둘러보기 '캠핑'이 쓰는 고캠핑 풀에서도 직접 검색(반려동물 API에 없는 캠핑장 보완).
    camp_hits = camp_search_in_region(name, prov_short, sigungu_name, dog, limit=8)
    ...
    if not KAKAO_REST_KEY:
        ...
        candidates = [p for p in candidates if _name_relevant(p.get("title"), name)]
        for p in candidates:
            ...
            if pet:
                ...
                p["inquiry"] = False
        # 둘러보기 풀에서 직접 찾은 곳을 합쳐, Kakao 미설정 상태에서도 누락을 막는다.
        _merge_pet_hits(candidates, pet_hits, dog)
        _merge_camp_hits(candidates, camp_hits)
        candidates = _suppress_dup_no_cards(candidates)
        _sort_verify_results(candidates, name)
        available = [c for c in candidates if c.get("verdict") in ("ok", "check", "conditional")]
        if not available:
            alts = verified_alternatives(pet_items, prov_short, sigungu_name, dog, limit=6)
            msg = ("입력한 장소를 찾지 못했어요." if not candidates
                   else "검색된 곳은 모두 반려견 동반이 어려워요. 대신 가까운 동반 가능한 곳을 추천해 드릴게요.")
    ...
    if not kakao_places and not pet_hits and not camp_hits:
        alts = verified_alternatives(pet_items, prov_short, sigungu_name, dog, limit=6)
    ...
    # 둘러보기 풀에서 직접 찾은 곳을 합친다.
    # → "둘러보기엔 있는데 검색하면 없음" 문제 해결(Kakao 미발견/이름 불일치 보완).
    _merge_pet_hits(results, pet_hits, dog)
    _merge_camp_hits(results, camp_hits)
    # 같은 장소가 '가능/확인 필요'로도, (Kakao발) '불가'로도 잡혔으면 모순 카드 제거.
    results = _suppress_dup_no_cards(results)

    _sort_verify_results(results, name)
    # 동반 가능/확인 필요가 하나라도 있으면 결과를 보여주고,
    # 검색 결과가 없거나 '전부 동반 불가'면 대신 동반 가능한 곳을 추천한다.
    available = [r for r in results if r.get("verdict") in ("ok", "check", "conditional")]
    if not available:
        alts = verified_alternatives(pet_items, prov_short, sigungu_name, dog, limit=6)
        msg = ("입력한 장소를 찾지 못했어요. 장소명을 확인해 주세요." if not results
               else "검색된 곳은 모두 반려견 동반이 어려워요. 대신 가까운 동반 가능한 곳을 추천해 드릴게요.")
        return jsonify({"success": True, "results": [], "query": name,
                        "alternatives": alts, "message": msg, "kakao_available": True})
...
def trip_save():
    ...
    doc["created"] = now_kst().strftime("%Y-%m-%d %H:%M")
